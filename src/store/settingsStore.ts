...
const LEGACY_DONATION_URL_KEY = "atelier-vecteur:donation-url";
...
interface SettingsState {
  ...
  showWelcomeOnStartup: boolean; donationUrl: string;
  setTheme: (theme: Theme) => void; toggleTheme: () => void; setLanguagePreference: (language: LanguagePreference) => void;
  setShowWelcomeOnStartup: (show: boolean) => void; setRemoteDonationUrl: (url: string) => void;
}
...
try { localStorage.removeItem(LEGACY_DONATION_URL_KEY); } catch { /* unavailable */ }
...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  showWelcomeOnStartup: initialShowWelcome, donationUrl: "",
  ...
  setRemoteDonationUrl: (donationUrl) => set({ donationUrl }),
}));
